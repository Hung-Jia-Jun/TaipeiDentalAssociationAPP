...
class Message extends Component {
    ...
    {
        _this.state.GroupUsers[item.key].toggled = !_this.state.GroupUsers[item.key].toggled;
        _this.setState({ToggleBtn : _this.state.GroupUsers});

        var _showGroupList = false;
        this.state.GroupUsers.forEach((e)=>{
            if (e.toggled == true)
            {
                _showGroupList = true;
                return;
    ...
    componentDidMount()
    {
        try
        {
            var _GroupUsers = JSON.parse(this.props.navigation.getParam('GroupUsers'));
            _GroupUsers.push({key:_GroupUsers.length.toString(),
                title : 'Append user',
                toggled:true,
            ...
            this.setState({GroupUsers : _GroupUsers});
        }
        catch (error)
        {
            console.log(error);
    ...
    {
        //TODO 群組設定要能追加與刪除成員，並且相應更新到用戶的群組資訊等內容內
        var dbRef = database.ref();
        //將時間戳當作ＩＤ
        var timeStamp = new Date().getTime().toString();
        var GroupID = this.state.GroupName+'_' + timeStamp;
        var GroupUser = []
        this.state.GroupUsers.forEach(user=>{
            //只把用戶選擇的客戶加進群組裡
    ...
    appendMember()
    {
        this.props.navigation.push('InviteGroupChat',{GroupUsers : JSON.stringify(this.state.GroupUsers) , ReferPage : 'GroupSetting'});
    }
  render() {
            ...
            <View style={{flex: 0.25,
				...
				<View style={{
					...
					<TouchableOpacity style={{
						...
						onPress={()=>this.props.navigation.push('GroupChat')}
						>
