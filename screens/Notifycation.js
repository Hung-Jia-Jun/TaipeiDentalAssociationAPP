	...
	updateGroupList()
	{
		...
		ref.on('value', function (snapshot) {
			...
			snapshot.forEach((childSnapshot) => {
				if (childSnapshot.val().groupType=="clinicNotify")
				{
					_GroupList.push({   key:i.toString(),
						title:childSnapshot.val().toUser,
						groupID:childSnapshot.val().chatID,
	...
	updateLastMsg()
	{
		...
		GroupList.forEach(e =>{
			...
			ref.child("group").child(groupID).get().then((snapshot) => {
				if  (snapshot.val().msg!=undefined && snapshot.val().groupType=='clinicNotify')
				{
...
const Group_Item = ({ _this,title,item,item_image,discription,groupID}) => (
	<View style={styles.container,{zIndex:0,
									flex: 1,
									flexDirection: 'row',
									height:item.groupType=='clinicNotify'?45:0,
		...
		<View style={{zIndex:0,width:Dimensions.get('window').width,height:50}}>
			<TouchableOpacity style={styles.button,{
					...
					<Image source={ item_image } style={{zIndex:0,
														...
														marginStart:50,
														width:30,
														height:item.groupType=='clinicNotify'?30:0,}}></Image>
					<Text style={{
						width:230,
						zIndex:0,
						height:item.groupType=='clinicNotify'?30:0,
						marginLeft: 100,
						textAlign:'left',
						marginTop:3,
						fontSize:15,
		...
		<ImageBackground style={{zIndex:0,marginTop:0,width:Dimensions.get('window').width,height:100,backgroundColor:'#F2FAFF'}}> 
			<View style={{zIndex:0,width:Dimensions.get('window').width,height:100}}>
				...
				<TouchableOpacity style={styles.button,{
						...
						zIndex:0,
						marginTop:0,
					}} onPress={() => _this.props.navigation.push('GroupChat',{GroupID : groupID , GroupName : title,groupType:'clinicNotify'})}>
					</TouchableOpacity>
			</View> 
		</ImageBackground>
	</View>
